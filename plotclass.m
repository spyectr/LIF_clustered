    ...
    methods(Static)
        ...
        function fun_PlotTrial(data,firings,Params)
            ...
            if plot_currents
                ...
                PSCPlot(Sim,Ext,iIplot*Jee,iEplot*Jee,iExtplot*Jee,VTh*Jee./(tau),ind_plot);
                xlim(XLIM);
                % SAVE PLOT
                ...
                if any(strcmp(fieldnames(Ext),'stim'))
                    ...
                    if any(strcmp(Params.stimuli,'CSgauss'))
                        ...
                        for i_g=1:numel(ind_gauss)
                            ...
                            %                             h=auxi.PlotHistStairs(distr,30,OPTIONS);
                            h=stairs(x,f,'linewidth',3,'color','m'); hold on;
            ...
            function PlotAllNeurons(Ext,firings,IN)
                ...
                utils.figset(gca,xlab,ylab,tt,15);
            ...
            function PSCPlot(Sim,Ext,iIplot,iEplot,iExtplot,VTh,ind_plot)
                binwidth=55;
                ...
                for j=1:nplot-1
                    ...
                    hold on;
                    [tout iISmooth] = utils.gaussfilt(t_plot,iIplot(j,:),binwidth,0);
                    [tout iESmooth] = utils.gaussfilt(t_plot,iEplot(j,:),binwidth,0);
                    [tout iExtSmooth] = utils.gaussfilt(t_plot,iExtplot(j,:),binwidth,0);
                    % [iISmooth]=auxi.eegfilt(iIplot(j,:),srate,0,hicutoff);
                    %                     [iESmooth]=auxi.eegfilt(iEplot(j,:),srate,0,hicutoff);
                    %                     [iExtSmooth]=auxi.eegfilt(iExtplot(j,:),srate,0,hicutoff);
                    ...
                    [tout itotSmooth] = utils.gaussfilt(t_plot,itot,binwidth,0);
                    %                     [itotSmooth]=auxi.eegfilt(itot,srate,0,hicutoff);
                    ...
                    if any(strcmp(fieldnames(Ext),'stim'))
                        for n=1:numel(Ext.stim)
                            ...
                            %                     [~,~]=auxi.jbfill(time_bin,lower,upper,'m',0,0,0.2);
            ...
            function [rate, x_bins]=PlotPopRates(bins,firings,N_e,N_i,NcE)
                ...
                fntsz=15;
                cols=utils.distinguishable_colors(numel(NcE)+1);
                binwidth=8;
                cusumNcE=[0 cumsum(NcE)'];
                ...
                for clu=1:numel(NcE)
                    ...
                    x=histc(spikes,bins);
                    [tout, z]=utils.gaussfilt(bins,x,binwidth,0);
                    x_bins=tout;
