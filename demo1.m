...
% 2) A network with E and I clusters (ClustersOption='EI')  [This part reproduces results from D. Wyrick et al., 2019]
%
...
auxi.create_params(paramsfile,Opt);
% if strcmp(ClustersOption,'E'); auxi.create_params(paramsfile);
% elseif strcmp(ClustersOption,'EI'); auxi.create_params_EI(paramsfile);
% end
...
% CUSTOM OPTIONS:
% 1) edit your custom-made stimuli inside auxi.create_params. Available options: 'US', 'CSgauss' from L. Mazzucato et al., Nat. Neuro. 2019
...
savedir='results'; if ~exist(savedir,'dir'); mkdir(savedir); end % setup directory for saving HMM data
...
% if strcmp(ClustersOption,'E'); [J, params]=auxi.fun_SynWeights(paramsfile);
% elseif strcmp(ClustersOption,'EI'); [J, params]=auxi.fun_SynWeights_EI(paramsfile);
% end
